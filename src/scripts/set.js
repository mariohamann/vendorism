 ...
 * @returns {Promise<{content: string, path: string}|undefined>} A promise that resolves with updated content and path after applying the transformation or undefined if no transformation was applied.
 */
    ...
    for (const file of files) {
      ...
      if (result) {
        if (!fs.existsSync(result.path)) {
          fs.mkdirSync(path.dirname(result.path), {
            recursive: true,
          });
          fs.writeFileSync(result.path, result.content, "utf8");
          // @ts-ignore
          output.newFiles.push(optimizePathForWindows(result.path));
        }
      }
    }
  }
